...
<div class="min-h-screen bg-gradient-to-br from-pink-100 via-purple-100 to-pink-200 py-10 px-4">
    <div class="max-w-xl mx-auto bg-white/80 backdrop-blur-md p-8 rounded-2xl shadow-xl">
        ...
        {{-- Título centralizado --}}
        <h2 class="text-3xl font-extrabold text-center text-gray-800 mb-6">Nova Plataforma</h2>

        {{-- Formulário --}}
        <form method="POST" action="{{ route('admin.plataformas.store') }}" class="space-y-5">
            @csrf
            ...
            <div class="pt-4 flex flex-col md:flex-row gap-4">
                <button type="submit"
                        class="w-full md:w-auto bg-pink-600 hover:bg-pink-700 text-white font-semibold py-2 px-6 rounded-lg shadow-md transition-all duration-300 ease-in-out transform hover:scale-105">
                    Salvar Plataforma
                </button>

                <a href="{{ route('admin.plataformas.index') }}"
                   class="w-full md:w-auto text-center bg-red-100 hover:bg-red-200 text-red-600 font-semibold py-2 px-6 rounded-lg shadow-md transition-all duration-300 ease-in-out transform hover:scale-105">
                    Cancelar
                </a>
            </div>
        </form>
